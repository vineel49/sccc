...
[P_State,P_Ip,Ga_Inx,N_State,Gb_Inx,~,~]= Get_Trellis();
...
%******************************************************************************
 for time=1:num_bit-1
     ...
     % backward recursion
     temp3 = log_beta(N_State(:,1),num_bit+2-time)+log(repmat_C(num_bit+1-time))+(1-2*(P_Ip(:,1)-1))*LLR(num_bit+1-time)/2+log_gamma(Gb_Inx(:,1),num_bit+1-time);
     temp4 = log_beta(N_State(:,2),num_bit+2-time)+log(repmat_C(num_bit+1-time))+(1-2*(P_Ip(:,2)-1))*LLR(num_bit+1-time)/2+log_gamma(Gb_Inx(:,2),num_bit+1-time);
